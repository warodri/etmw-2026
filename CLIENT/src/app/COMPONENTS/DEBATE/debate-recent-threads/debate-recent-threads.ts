...
@Component({
    selector: 'app-debate-recent-threads',
    standalone: false,
    templateUrl: './debate-recent-threads.html',
    styleUrl: './debate-recent-threads.css',
})
export class DebateRecentThreads implements OnInit, OnChanges {
    language: 'en' | 'es' = 'en';
    ...
    recentThreads = signal<DebateCommentModel[]>([]);
    ...
    loadRecentThreads() {
        ...
        this.error.set(null);
        this.iDebate.debateGetRecentThreads(audiobookId, this.limit, (response: any) => {
            this.loading.set(false);
            if (response && response.success) {
                this.recentThreads.set((response.threads || []) as DebateCommentModel[]);
                return;
            }
            this.error.set(response?.message || this.tr('Unable to load recent threads', 'No se pudieron cargar los hilos recientes'));
            this.recentThreads.set([]);
        });
    }
